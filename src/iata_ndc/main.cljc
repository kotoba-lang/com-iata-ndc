(ns iata_ndc.main
  "Kotodama WASM entrypoint for the Iata_ndc clean-room actor (L4) — Clojure port.
  ...
  facts (`iata_ndc.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "iata_ndc")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"iata_ndc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iata_ndc-compat" :tier tier :entities entities} 200])
...
(defn start! [] :iata_ndc-compat/ready)
